    ...
    for k, v in inputs.items():
        if str(v).startswith("task."):
            ...
            if upstream_task not in config.sym_table:
                raise ConfigurationError(
                    "Task {0} defines an upstream task {1} that does not exist.".format(
                        name, v
                    )
                )
            ...
            if defined_dependency not in upstream_outputs:
                raise ConfigurationError(
                    ...
                    + "defined as an output on task {3}.".format(
                        k, v, name, upstream_task
                    )
                )
        ...
        if str(v).startswith("var."):
            ...
            if defined_var not in config.var:
                raise ConfigurationError(
                    "The input {0} : {1} defined in task {2} does not exist, check 'var'".format(
                        k, v, name
                    )
                )
...
def build_symbol_entry(config: OrcaConfig, task: Dict):
    name = task.get("task", None)
    return name, {"id": check_unique(config, name, task), "definition": task}
...
def check_variables(config: OrcaConfig):
    ...
    for key, val in config.var.items():
        if not key.isidentifier():
            raise ConfigurationError('Invalid variable identifier: "{0}"'.format(key))
...
def check_url(url):
    try:
        ...
        if r.status_code != 200:
            raise ConfigurationError(
                "The url provided {0} was not accessible.".format(url)
            )
