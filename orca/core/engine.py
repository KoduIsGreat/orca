...
def __get_call_string(config: Dict, script: str):
    ...
    pattern = (
        r"((?P<keyword>def)\s?(?P<function>\w+)\s?\((?P<args>(?P<arg>\w+(,\s?)?)+)?\))"
    )
...
def clean_locals(task: OrcaTask):
    keys_to_remove = [
        k for k in task.locals if k not in task.outputs and k not in task.inputs
    ]
    ...
    try:
        ...
        if resolved_file is None:
            ...
            with open(resolved_file, "r") as script:
                _task.locals[
                    "__file__"
                ] = resolved_file  # if they want to reference __file__ in their script
...
def execute_http(task: OrcaTask) -> None:
        ...
        try:
            switch = {
                "GET": lambda _task: requests.get(
                    _task.http, _task.locals, verify=False
                ),
                "PUT": lambda _task, data: requests.put(
                    _task.http, _task.locals, verify=False
                ),
                "POST": lambda _task: requests.post(
                    _task.http, json=_task.locals, verify=False
                ),
    ...
    if http_response.status_code >= 400:
        ...
        raise ExecutionError(
            ...
            + "200 status\n status code: {1}\n url: {2}".format(
                task.name, http_response.status_code, task.http
            )
        )
...
def __resolve_task__(
    task: Dict, config: OrcaConfig, snapshot=None, run_globals={}
) -> OrcaTask:
    ...
    if inputs:
        ...
        for k, v in inputs.items():
            if str(v).startswith("task."):
                ...
                _locals[k] = config.cache.fetch(
                    upstream_task, filters=split[2:], snapshot=snapshot
                )
            elif str(v).startswith("var."):
                split = str(v).split(".")
                defined_var = split[1]
                _locals[k] = eval(
                    str(config.var[defined_var]), globals()
                )  # this should change
            else:
...
def __run_task__(
    task: Dict, config: OrcaConfig, snapshot=None, ledger: Ledger = None, run_globals={}
):
        ...
        for task in task_queue:
            ...
            if isinstance(task, list):
                ...
                with ThreadPoolExecutor(max_workers=len(concurrent_jobs)) as executor:
                    futures = [
                        executor.submit(execute_job, job, _config, _ledger, snapshot)
