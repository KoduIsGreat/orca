...
class OrcaCache:
    ...
    def fetch(self, task_name, filters=(), snapshot=None):
        if task_name not in self.mem_cache:
            self.mem_cache[task_name] = self.workflow_file_cache.task(
                task_name, snapshot=snapshot
            ).data
        if filters:
    ...
    def store(self, _task: OrcaTask):
        ...
        self.workflow_file_cache.write(
            _task.name, _task.locals, meta=_task.task_data, overwrite=True
        )
