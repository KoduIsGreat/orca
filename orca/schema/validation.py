...
def _handle_errors(errors: List[ValidationError], fmt_err_func, filename):
    ...
    raise ConfigurationError(
        "The Orca configuration: {file} is invalid because:\n {error_msg}".format(
            file=" ' {}'".format(filename) if filename else "", error_msg=error_msg
        )
    )
...
def _parse_anyof_validator(error: ValidationError):
    ...
    for error in errors:
        ...
        if error.validator == "uniqueItems":
            return (
                error.absolute_path if error.path else None,
        ...
        if error.validator == "type":
            return (
                error.absolute_path,
                "An invalid type was declared: {0}".format(error.message),
            )
...
def validate(file: TextIO):
    ...
    schema_file = os.path.join(
        _get_schema_location(), "ORCA_SCHEMA_{0}.json".format(version)
    )
